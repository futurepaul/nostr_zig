...
export fn wasm_sign_schnorr(
    ...
    // The static context doesn't have SIGN capabilities needed for signature creation
    const ctx = secp256k1.secp256k1_context_create(secp256k1.SECP256K1_CONTEXT_SIGN) orelse return false;
...
// Event creation and verification exports
...
export fn wasm_create_event(
    ...
    // Parse tags array - use arena allocator to ensure tags persist
    ...
    // Create event using EventBuilder with arena allocator
...
/// Get public key from private key with hex output (for event creation)
export fn wasm_get_public_key_hex(
    private_key: [*]const u8,  // 32 bytes
    out_pubkey_hex: [*]u8,     // 64 bytes hex
) bool {
    const priv_key = private_key[0..32].*;
