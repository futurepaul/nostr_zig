...
fn deriveMlsPublicKey(allocator: std.mem.Allocator, nostr_pubkey: [32]u8) ![]u8 {
    ...
    const prk = try crypto_utils.hkdfExtract(allocator, constants.HKDF_SALT.NOSTR_TO_MLS_ID, &nostr_pubkey);
    defer allocator.free(prk);
    
    // Creates a deterministic 32-byte identifier
    return try crypto_utils.hkdfExpand(allocator, prk, constants.HKDF_INFO.MLS_IDENTIFIER, constants.KEY_SIZES.HKDF_OUTPUT);
}
...
fn createSenderData(allocator: std.mem.Allocator, sender: types.Sender) ![]u8 {
    ...
    switch (sender) {
        .member => |idx| {
            ...
            std.mem.writeInt(u32, &idx_bytes, idx, .little);
            try buf.appendSlice(&idx_bytes);
        },
        else => {},
    }
...
fn parseSenderData(data: []const u8) !types.Sender {
    ...
    return switch (sender_type) {
        .member => blk: {
            ...
            const idx = std.mem.readInt(u32, data[1..5], .little);
