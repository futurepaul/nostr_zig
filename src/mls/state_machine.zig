...
pub const MLSStateMachine = struct {
    ...
    /// Transcript hashes for maintaining group history integrity
    confirmed_transcript_hash: [32]u8 = [_]u8{0} ** 32,  // Running hash of all Commits
    interim_transcript_hash: [32]u8 = [_]u8{0} ** 32,    // Includes confirmation tag
    ...
    pub fn initializeGroup(
        ...
        rotation_policy: KeyRotationPolicy,
        epoch_secrets: ?mls.EpochSecrets, // Optional epoch secrets to use instead of placeholders
    ) !MLSStateMachine {
        ...
        // Use provided epoch secrets or extract from MLS group
        const secure_epoch_secrets = if (epoch_secrets) |provided_secrets| blk: {
            // Use the epoch secrets provided by the caller (from groups.zig)
            break :blk forward_secrecy.SecureEpochSecrets.init(provided_secrets);
        } else if (mls_group.epoch_secrets) |mls_secrets| blk: {
            const real_epoch_secrets = mls.EpochSecrets{
                .joiner_secret = @bitCast(mls_secrets.joiner_secret.data[0..32].*),
                .member_secret = @bitCast(mls_secrets.epoch_secret.data[0..32].*),
                .welcome_secret = [_]u8{0} ** 32, // Not in mls_zig struct
                .epoch_secret = @bitCast(mls_secrets.epoch_secret.data[0..32].*),
                .sender_data_secret = @bitCast(mls_secrets.sender_data_secret.data[0..32].*),
                .encryption_secret = @bitCast(mls_secrets.encryption_secret.data[0..32].*),
                .exporter_secret = @bitCast(mls_secrets.exporter_secret.data[0..32].*),
                .epoch_authenticator = @bitCast(mls_secrets.authentication_secret.data[0..32].*),
                .external_secret = @bitCast(mls_secrets.external_secret.data[0..32].*),
                .confirmation_key = @bitCast(mls_secrets.confirmation_key.data[0..32].*),
                .membership_key = @bitCast(mls_secrets.membership_key.data[0..32].*),
                .resumption_psk = @bitCast(mls_secrets.resumption_psk.data[0..32].*),
    ...
    pub fn joinFromWelcome(
        ...
        // Use provided epoch secrets or extract from MLS group
        const secure_epoch_secrets = if (epoch_secrets) |provided_secrets| blk: {
            // Use the epoch secrets provided by the caller (from welcomes.zig)
            break :blk forward_secrecy.SecureEpochSecrets.init(provided_secrets);
        } else if (mls_group.epoch_secrets) |mls_secrets| blk: {
            const real_epoch_secrets = mls.EpochSecrets{
                .joiner_secret = @bitCast(mls_secrets.joiner_secret.data[0..32].*),
    ...
    pub fn commitProposals(self: *MLSStateMachine, committer_index: u32, mls_provider: anytype) !CommitResult {
        ...
        if (self.mls_group) |*group| {
            ...
            // Update transcript hashes when epoch advances
            if (secrets_rotated) {
                // In a real implementation, we would:
                // 1. Serialize the AuthenticatedContent of the Commit
                // 2. Update confirmed_transcript_hash = Hash(old_hash || commit_content)
                // 3. Compute interim_transcript_hash with confirmation tag
                //
                // For now, we simulate the update with a deterministic hash
                const groups = @import("groups.zig");
                const pseudo_commit_content = try std.fmt.allocPrint(
                    self.allocator,
                    "commit_epoch_{}_members_{}",
                    .{ new_epoch, self.members.items.len }
                );
                defer self.allocator.free(pseudo_commit_content);
                
                // Update confirmed transcript hash
                self.confirmed_transcript_hash = try groups.updateTranscriptHashWithCommit(
                    self.allocator,
                    mls_provider,
                    self.confirmed_transcript_hash,
                    pseudo_commit_content,
                );
                
                // Update interim transcript hash (would include confirmation tag)
                self.interim_transcript_hash = self.confirmed_transcript_hash; // Simplified
            }
